...
[obslik2, mixmat, fwd_only, scaled, act, maximize, compute_xi, compute_gamma2, handle_zero_lik] = ...
...
%%
p1 = 0;
GetRandAlpha = @(a,b) rand(size(a)) .* b;
t = 1;
alpha(:,1) = init_state_distrib(:) .* obslik(:,t) + GetRandAlpha(alpha(:,1), 1e-5);
if scaled
...
%         [alpha(:,t), scale(t)] = normaliseC(alpha(:,t));
    ...
    % if ~approxeq(sum(alpha(:,t)),1) % handle_zero_lik
    if ~((1 - sum(alpha(:,t))) < 1e-5)
        if ~p1
            p1 = 1;
            fprintf('All zero: %d\n', t);
        end
        
        % Set to previous estimate if liklihood is 0
%         if ~approxeq(sum(alpha(:,t)),1)
            r = GetRandAlpha(alpha(:,t-1), 1e-4);
            [alpha(:,t),scale(t)] = normalise( alpha(:,t-1) + r );
%         end
    end
%     assert(approxeq(sum(alpha(:,t)),1))
end
...
sumAlpha = sum(alpha);
sumAddToOne = approxeq(sumAlpha,ones(size(sumAlpha)));
if ~sumAddToOne
    numInvalid = sum(sumAlpha~=1);
    total = length(sumAlpha);
    error('Alpha did not sum to be 1 (%d of %d instances). Your data or coefficients may be invalid.', numInvalid, total);
end
%%
if scaled
    if any(scale==0)
