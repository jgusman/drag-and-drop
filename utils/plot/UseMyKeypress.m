function UseMyKeypress(hFig, myKeypressFun, customModes, callbackFcn)
...
%             'PostSet', @(obj,event) HandleModeUpdate(obj,event,hFig,myKeypressFun,customModes));
    ...
    hListener = event.proplistener( hManager, prop, ...
            'PostSet', @(obj,evnt) HandleModeUpdate(obj,evnt,hFig,myKeypressFun,customModes, callbackFcn));

    % Save so the listener persists
    mkl = getappdata(0,'mykeypress_listeners');
    listernFieldname = sprintf('fig%d%s', hFig.Number,callbackFcn);
    mkl.(listernFieldname) = hListener;
    setappdata(0,'mykeypress_listeners',mkl); 


%% Disable Mode listeners and set my keypress function
    evnt.AffectedObject = hManager;
    HandleModeUpdate([],evnt, hFig, myKeypressFun, customModes, callbackFcn)
...
function HandleModeUpdate(obj,evnt, hFig, myKeypressFun, customModes, callbackFcn)
...
%     hFig = hManager.CurrentMode.FigureHandle;
    ...
    if isempty(hManager.CurrentMode) || isempty(customModes)
        isCustom = [];
    else
    ...
    if isempty(isCustom)
        ...
        LocalUseMyKeypress(callbackFcn, hManager,hFig,myKeypressFun); 
        ...
        switch customMode
            ...
            case 'append'
                isAppend = 1;
                LocalUseMyKeypress(callbackFcn, hManager,hFig,myKeypressFun, isAppend)
...
function LocalUseMyKeypress(callbackFcn, hManager,hFig,myKeypressFun, isAppend)
...
% https://undocumentedmatlab.com/articles/enabling-user-callbacks-during-zoom-pan
    ...
    if ~isAppend
        % Default
        set(hFig, callbackFcn, [], 'KeyPressFcn', myKeypressFun);    
        
    else
        % Append
        windowCB = get(hFig, callbackFcn);
        if iscell( windowCB )
